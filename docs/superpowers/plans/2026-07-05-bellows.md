...
The `node --test <directory>` form documented above (Global Constraints and Tasks 2, 3, 4, 11)
fails on Windows with Node 22: the test runner treats each file under the directory as an entry
module rather than a test file, so it does not collect and run the `test()` cases as expected.
The form that actually works is explicit test-file paths, for example:

`node --test plugins/bellows/scripts/nim-lib.test.mjs plugins/bellows/scripts/nim-cli.test.mjs plugins/bellows/mcp/server.test.mjs`

This is the form the shipped `plugins/bellows/README.md` documents under Tests. Treat the
directory form in this plan as historical intent, not a working command on this platform.
