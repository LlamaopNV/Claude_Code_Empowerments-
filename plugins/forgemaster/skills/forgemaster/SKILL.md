...
2. **Done is a measured state, not a feeling.** The run manifest's six gates flip to `pass` only on observed evidence, and the `done-gate` hook physically blocks marking the run done while any gate is unmet.
...
forgemaster-runs/YYYY-MM-DD-<slug>/
  run.json          # the manifest: state machine + gate ledger (schema below)
  ...
  06-gates.md       # evidence per gate: command, observed output, verdict
  07-summary.md     # the delivery report
```

`run.json` (keep this exact flat shape — the hook greps it):
...
```json
{
  ...
  "stage": "intake | diverge | spec | plan | build | gates | deliver",
  "status": "active | paused | done | abandoned",
  "checkpoint": "pause | auto",
  "artifacts": { "<stage>": "<filename or skipped>" },
  "skips": [ { "stage": "", "reason": "" } ],
  "gates": {
    "tests": "pending", "lint": "pending", "types": "pending",
    "spec_review": "pending", "code_review": "pending", "self_critique": "pending"
  }
}
```

Update `stage` when entering a stage and the matching `artifacts` entry when leaving it. Gate values only ever become `pass` (observed evidence in `06-gates.md`), `fail` (loop back and fix), or `na` (impossible for this deliverable — reason in `06-gates.md`; e.g. `types` for a plain-bash deliverable).
...
Run each gate, write the **command and observed output** into `06-gates.md`, then flip the manifest value. Any `fail` → fix → re-run that gate. The gates:
...
If the deliverable is itself a Claude Code artifact (skill/plugin/agent), add `proofmark` as a seventh check and suggest an `anvil` eval after delivery.

### 6 — Deliver

`superpowers:finishing-a-development-branch` for the merge/PR decision. Write `07-summary.md`: what was built, the key decisions (each linking the stage artifact that made it), how it was verified (gate evidence), and how to run it. Only then set `status` to `done` — the hook will hold you to the gate ledger.

## Resuming

"Continue the dupescan run" / `/forgemaster resume`: find the newest `forgemaster-runs/*/run.json` with status `active` or `paused`, announce the stage, and re-enter it **from the artifacts, not from memory**. Any artifact can be regenerated by re-running its stage; downstream artifacts are then stale and their stages re-run.
...
- Do not flip a gate to `pass` without the observed evidence line in `06-gates.md`.
...
- Do not mark the run `done` with unmet gates — the hook blocks it; the answer is to fix, not to route around the manifest.

## Red flags — STOP

| Thought | Reality |
|---------|---------|
| "The user said skip the ceremony" | Ceremony ≠ record. Scale the *depth* (weight class, recorded skips), never the artifacts, tests-first, or gates. |
