...
bb_get_raw() {
  _bb_curl -L -H "Authorization: $(_bb_auth)" "$1"
}

# POST JSON: bb_post <url> [json_body]
# Only set Content-Type when a body is provided. Bitbucket endpoints like
# /approve, /request-changes, /unapprove reject empty-body POSTs that carry
# Content-Type: application/json with HTTP 400.
bb_post() {
  local args=(-X POST -H "Authorization: $(_bb_auth)" -H "Accept: application/json")
  if [[ -n "${2:-}" ]]; then
    args+=(-H "Content-Type: application/json" -d "$2")
  fi
  _bb_curl "${args[@]}" "$1"
}
